...
private[dap] final class DebugAdapterProxy(
  ...
  private val (observer, events) =
    Observable.multicast[Messages.Event](MulticastStrategy.replay)(ioScheduler)
...
private[dap] object DebugAdapterProxy {
  def apply(socket: Socket)(ioScheduler: Scheduler): DebugAdapterProxy = {
    val in = BaseProtocolMessage
      .fromInputStream(socket.getInputStream, null)
      .liftByOperator(Parser)

    val out: Writer = ??? // new Writer(BloopLanguageClient.fromOutputStream(socket.getOutputStream, null))

    new DebugAdapterProxy(in, out, ioScheduler)
  }
